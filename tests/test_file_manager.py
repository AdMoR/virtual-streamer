...
class TestFilePathConfig:
    ...
    def test_subdirectory_defaults(self, test_config):
        ...
        assert test_config.subtitle_temp_dir == os.path.join(
            test_config.temp_dir, "subtitles"
        )
        assert test_config.concat_temp_dir == os.path.join(
            test_config.temp_dir, "concat"
        )
    ...
    def test_custom_subdirectories(self, temp_test_dir):
        ...
        config = FilePathConfig(temp_dir=temp_test_dir, audio_temp_dir=custom_audio)
    ...
    def test_get_category_dir(self, test_config):
        ...
        assert (
            test_config.get_category_dir(FileCategory.AUDIO)
            == test_config.audio_temp_dir
        )
        assert (
            test_config.get_category_dir(FileCategory.VIDEO)
            == test_config.video_temp_dir
        )
        assert (
            test_config.get_category_dir(FileCategory.SUBTITLE)
            == test_config.subtitle_temp_dir
        )
        assert (
            test_config.get_category_dir(FileCategory.OUTPUT, temp=False)
...
class TestFileNamingStrategy:
    ...
    def test_generate_filename_basic(self, test_config):
        ...
        filename = naming.generate_filename(
            "test", "txt", include_timestamp=False, include_uuid=False
        )
    ...
    def test_generate_filename_with_suffix(self, test_config):
        ...
        filename = naming.generate_filename(
            "test", "txt", suffix="data", include_timestamp=False, include_uuid=False
        )
    ...
    def test_generate_filename_with_timestamp(self, test_config):
        ...
        filename = naming.generate_filename(
            "test", "txt", include_timestamp=True, include_uuid=False
        )
    ...
    def test_generate_filename_with_uuid(self, test_config):
        ...
        filename = naming.generate_filename(
            "test", "txt", include_timestamp=False, include_uuid=True
        )
...
class TestFileTracker:
    ...
    def test_initialization(self, temp_test_dir):
        ...
        tracker = FileTracker(
            filepath=filepath, category=FileCategory.AUDIO, purpose="Test file"
        )
...
class TestFileManager:
    ...
    def test_get_temp_path_with_subdirs(self, file_manager, test_config):
        ...
        path = file_manager.get_temp_path(
            "subdir/test.wav", FileCategory.AUDIO, create_dir=True
        )
    ...
    def test_register_file(self, file_manager):
        ...
        tracker = file_manager.register_file(
            filepath, FileCategory.AUDIO, purpose="Test audio", max_age_hours=2
        )
    ...
    def test_cleanup_expired_files_disabled(self, test_config, temp_test_dir):
        ...
        tracker = file_manager.register_file(
            filepath, FileCategory.TEMP, max_age_hours=0
        )
    ...
    def test_cleanup_expired_files(self, file_manager, temp_test_dir):
        ...
        tracker = file_manager.register_file(
            filepath, FileCategory.TEMP, max_age_hours=1
        )
        tracker.created_at = datetime.now() - timedelta(hours=2)
    ...
    def test_cleanup_expired_files_respects_should_cleanup(
        self, file_manager, temp_test_dir
    ):
        ...
        tracker = file_manager.register_file(
            filepath, FileCategory.TEMP, max_age_hours=1, should_cleanup=False
        )
        tracker.created_at = datetime.now() - timedelta(hours=2)
...
class TestIntegration:
    ...
    def test_complete_workflow(self, file_manager, temp_test_dir):
        ...
        tracker = file_manager.register_file(
            filepath, FileCategory.AUDIO, purpose="TTS output", max_age_hours=1
        )
