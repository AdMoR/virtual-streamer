...
def main():
    ...
    parser.add_argument("config_dump", nargs="?", help="Path to config dump JSON file")
    parser.add_argument(
        "--output", "-o", help="Output HTML file path (default: auto-generated)"
    )
    parser.add_argument(
        "--latest",
        action="store_true",
    ...
    if args.latest:
        ...
        print(
            "Error: Please provide a config dump file or use --latest", file=sys.stderr
        )
        parser.print_help()
        return 1
    ...
    try:
        ...
        html_path = create_html_report_from_dump(
            config_dump_path, output_path=args.output
        )
        ...
        if args.open:
            ...
            print(
                f"   Or run: python scripts/create_report.py {config_dump_path} --open"
            )
