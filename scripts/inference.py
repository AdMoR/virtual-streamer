...
from virtual_streamer.workflows.prompts import (
    PROMPT,
    PROMPT_FR,
    PROMPT_FR_3,
    PROMPT_FR_2,
...
def main(question):
    ...
    if question.prompt is None:
        question.prompt = random.choice(
            [SARCASTIC_STANDUP, VERY_SARCASTIC_STANDUP_PROMPT, VERY_SARCASTIC_PROMPT]
        )
    ...
    question_data = {
        ...
        "subtitle_mode": question.subtitle_mode.name
        if hasattr(question.subtitle_mode, "name")
        else question.subtitle_mode,
        "name": question.name,
    }

    payload = {"question": question_data, "gpt_response": text}
...
def callback(ch, method_frame, properties, body):
    ...
    if media_path.startswith("http"):
        ...
        s3_path = (
            s3_upload(media_path, UPLOAD_BUCKET)
