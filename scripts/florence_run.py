...
flo_model = AutoModelForCausalLM.from_pretrained(model_id, trust_remote_code=True, torch_dtype='auto').eval().cuda()
...
def run_example(task_prompt, image_, text_input=None):
    ...
    inputs = flo_processor(text=prompt, images=image_, return_tensors="pt").to('cuda', torch.float16)
    ...
    generated_text = flo_processor.batch_decode(generated_ids, skip_special_tokens=False)[0]
    parsed_answer = flo_processor.post_process_generation(
        generated_text, 
        task=task_prompt, 
        image_size=(image_.shape[1], image_.shape[0])
    )
