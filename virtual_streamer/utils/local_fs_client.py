    ...
    async def s3_list_keys(self, prefix: str) -> List[str]:
        ...
        if self.base_path not in prefix_path.parents and prefix_path != self.base_path:
            ...
            if str(prefix_path).startswith(str(self.base_path)):
                ...
                print(
                    f"List prefix '{prefix}' resolves outside base path '{self.base_path}'. Returning empty list."
                )
        ...
        if not prefix_path.is_dir():
            ...
            if prefix_path.is_file() and str(prefix_path).startswith(
                str(self.base_path / prefix)
            ):
                try:
                    relative_path = prefix_path.relative_to(self.base_path)
                    return [
                        str(relative_path).replace(os.sep, "/")
                    ]  # Normalize to forward slashes like S3 keys
                except ValueError:
        ...
        for item in prefix_path.rglob("*"):  # Recursive glob
            if item.is_file():
                ...
                try:
                    ...
                    print(
                        f"Warning: Found item {item} outside base path during listing?"
                    )
                    continue
