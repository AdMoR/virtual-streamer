...
Mirrors the StoryRepository pattern: lazy pool initialisation, autocommit=True,
and a _ensure_tables() call on first connection. Binary artifacts remain in
MinIO (prefix eval/{run_id}/); only metadata and keys are stored here.
...
class EvalRepository:
    ...
    def __init__(
        ...
        print(f"Initialized EvalRepository for {self.host}:{self.port}/{self.database}")
    ...
    async def _ensure_tables(self):
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                print("Ensured eval tables exist")
    ...
    async def list_benches(self) -> List[Dict[str, Any]]:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                return [self._bench_row_to_dict(r) for r in await cur.fetchall()]
    ...
    async def list_samples_for_run(self, run_id: str) -> List[Dict[str, Any]]:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                return [self._sample_row_to_dict(r) for r in await cur.fetchall()]
