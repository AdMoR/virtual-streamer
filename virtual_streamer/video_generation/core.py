...
async def generate_search_keyword(
    ...
    prompt = KEYWORD_GENERATION_PROMPT.format(
        previous_keywords=previous_str,
        dialogue=dialogue
    )
...
async def find_best_video_for_sentence(
    ...
    for j in valid_judgements:
        if j.rating == "CONTEXTUAL":
            best_judgement = j
            break
        elif j.rating == "NEUTRAL" and (not best_judgement or best_judgement.rating == "NOT_CONTEXTUAL"):
            best_judgement = j
    ...
    if best_judgement and best_judgement.rating == "NOT_CONTEXTUAL":
        ...
        for keyword in keywords:
            ...
            if alt_videos:
                ...
                if best_judgement and best_judgement.rating in ["CONTEXTUAL", "NEUTRAL"]:
                    break
        ...
        execution={
            ...
            "sentences_with_details": [
                ...
                for i, (sent, match, audio, subtitle, segment) in enumerate(
                    zip(sentences, video_matches, audio_files, subtitle_files, video_segments)
                )
            ]
        },
        output={
...
async def recreate_from_config_dump(
    ...
    video_segments = []
    for i, (video, audio, subtitle) in enumerate(zip(video_paths, audio_files, subtitle_files)):
        if progress:
