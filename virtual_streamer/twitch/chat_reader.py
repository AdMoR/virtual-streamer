...
class TwitchClient:
    ...
    def refresh_access_token(self):
        ...
        try:
            with self.token_lock:
                ...
                logger.info(f"Access token refreshed, expires in {response_data['expires_in']} seconds")
                return True
        ...
        try:
            if os.path.exists(self.token_file):
                with open(self.token_file, 'r') as f:
                    ...
                    logger.info(f"Loaded refresh token from file (updated: {data['updated_at']})")
                    return True
            return False
        except Exception as e:
    ...
    async def connect_to_chat(self, channel_name):
        ...
        try:
            ...
            if e.status_code == 400:  # Bad Request, likely due to invalid token
                logger.warning(f"Invalid status code {e.status_code}, refreshing token...")
                self.refresh_access_token()
            else:
    ...
    def get_stream_chat(self, channel_name):
        ...
        async def read_chat():
            ...
            while True:
                try:
                    logger.info(f"Connecting to Twitch chat for channel: {channel_name}")
    ...
    def handle_privmsg(self, websocket, message, channel):
        ...
        try:
            ...
            if chat_message.lstrip(" ").lower().startswith("!allo") or chat_message.lstrip(" ").lower().startswith("allo"):
                question_text = chat_message[5:].strip() if chat_message.lstrip(" ").lower().startswith("!allo") else chat_message[4:].strip()
